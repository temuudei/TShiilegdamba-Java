...
public class ForageService {
    ...
    private void checkDuplicates(Forage forage, Result<Forage> result) {
        ...
        if (duplicates.stream().filter(f -> f.getForager().getLastName().equalsIgnoreCase(forage.getForager().getLastName()) &&
                                            f.getForager().getFirstName().equalsIgnoreCase(forage.getForager().getFirstName()))
                               .anyMatch(f -> f.getItem().getName().equalsIgnoreCase(forage.getItem().getName()))) {
            result.addErrorMessage("" + forage.getForager().getFirstName() + " " + forage.getForager().getLastName()
                                      + " has already foraged " + forage.getItem().getName()
                                      + " on this date. Duplicates are not allowed.");
        }
    }
}
